...
class StatsDataSource {
  static Future<List<Transaction>> getAllTransactions() async {
    ...
    for (final t in manual) unique[t.id] = t;
    for (final t in sms) {
  ...
  static Future<List<Transaction>> getFilteredTransactions(DateTime month) async {
    final all = await getAllTransactions();
    return all.where((t) => t.date.year == month.year && t.date.month == month.month).toList();
  }
      ...
      return smsList.map((smsT) {
        ...
        if (isTransfer) {
          if (smsT.isCredit) {
            ...
            if (upper.contains('TO ACCOUNT') || upper.contains('TRANSFERRED TO') ||
                upper.contains('NEFT') || upper.contains('RTGS') || upper.contains('IMPS') || upper.contains('UPI')) {
        ...
        final title = isTransfer
            ? (smsT.isCredit
                ...
                : (upper.contains('TO ACCOUNT') || upper.contains('TRANSFERRED TO') ||
                        upper.contains('NEFT') || upper.contains('RTGS') || upper.contains('IMPS')
                    ? '${smsT.bankName} Transfer'
                    : '${smsT.bankName} Withdrawal'))
            : '${smsT.bankName} ${smsT.isCredit ? "Credit" : "Debit"}';
        final note = 'From SMS: ${smsT.rawMessage.length > 50 ? '${smsT.rawMessage.substring(0, 50)}...' : smsT.rawMessage}';
        return Transaction(
          id: 'sms_${smsT.id}',
          title: title,
          amount: smsT.amount,
          type: isTransfer ? TransactionType.transfer : (smsT.isCredit ? TransactionType.income : TransactionType.expense),
          date: smsT.date,
  ...
  static bool _isSmsTransactionTransfer(ParsedSmsTransaction smsT) {
    ...
    const expenseKeywords = [
      'PAYMENT', 'PAID', 'PURCHASE', 'PURCHASED', 'BILL', 'MERCHANT', 'POS', 'DEBIT CARD', 'CREDIT CARD',
      'ONLINE', 'SHOPPING', 'RESTAURANT', 'FOOD', 'GROCERY', 'FUEL', 'PETROL', 'DIESEL', 'TAXI', 'UBER',
      'OLA', 'RENT', 'SALARY', 'SERVICE', 'CHARGE', 'FEE', 'TAX',
    ];
  ...
  static String? _extractRecipientAccount(String message) {
    ...
    for (final p in patterns) {
      ...
      if (m != null && m.group(1) != null) {
        final acc = m.group(1)!;
        return acc.contains('@') ? acc : '****${acc.length > 4 ? acc.substring(acc.length - 4) : acc}';
      }
    }
    return null;
  }
